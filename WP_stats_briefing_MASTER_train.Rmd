...
library(XLConnect)
non_cohort <- "./Briefing Dec 2015/work-programme-time-series-to-sept-2015/"
cohort <- "./Briefing Dec 2015/work-programme-cohort-data-to-sept-2015-csv/"
...
c <- ggplot(fortified, aes(x=Index, y=Value, fill= Series)) + geom_area() + theme_economist_white(base_size=12, gray_bg = FALSE) + scale_fill_lwi() + guides(fill=guide_legend(nrow=3)) + scale_y_continuous(labels=comma) + labs(y="Monthly referrals", x= "Month of referral")
...
ggmoney <- ggplot(money, aes(x=Index, y=Value, fill=Series)) + geom_area() + theme_economist_white(base_size=12, gray_bg=FALSE) + scale_fill_lwi() + labs(x="", y="") + scale_y_continuous(labels=dollar_format(prefix = "\u00A3")) 
```
...
p <- p + theme_economist_white(gray_bg = FALSE) + scale_y_continuous(limits=c(0, 0.4), labels=percent) + scale_colour_lwi()
...
ggfig1 <- ggplot(fig1df_melted, aes(x=measure, y=value, fill=variable)) + geom_bar(stat="identity", position = "dodge") + 
  geom_linerange(aes(x=measure, ymin=value, ymax = value + fig1df_errorbar_melted$value), position=position_dodge(0.9), colour="#ee7e3b", alpha=0.8, size=12) + geom_text(aes(y=value, label=percent(value)),position=position_dodge(0.9), hjust=0) +
...
ggfig2 <- autoplot.zoo(joperc, facets=NULL) + 
  ...
  scale_colour_lwi() + scale_y_continuous(labels=percent, limits=c(0,0.4)) 
...
completer_plot <- autoplot.zoo(head(zoo_completers[,11:12],-27), facets=NULL) + 
  ...
  scale_colour_discrete(labels=c("Completed with job", "Returned to Jobcentre Plus")) + scale_colour_lwi() + 
  geom_line(size=2) +
